...
public class CollectibleItem : MonoBehaviour
{
    ...
    public float respawnTime = 30.0f;           //리스폰 시간(아이템이 다시 생성될 때 까지의 대기 시간)
    ...
    {
        canCollect = false;         //수집 불가능한 상태로 변경
        GetComponent<MeshRenderer>().enabled = false;           //아티메의 MeshRenderer를 꺼서 보이지 않게 함
        GetComponent<MeshCollider>().enabled = false;           //아티메의 MeshCollider를 꺼서 보이지 않게 함

        //설ㅈㅓㅇ된 리스폰 시간 만ㅋㅡㅁ 대기
        yield return new WaitForSeconds(respawnTime);

        GetComponent<MeshRenderer>().enabled = true;            //아이템의 MeshRenderer를 다시 보이게 함
        GetComponent<MeshCollider>().enabled = true;            //아이템의 MeshCollider를 다시 보이게 함
